...
<html>
  ...
  <body>
    ...
    <h3><div id="bal" align="center">Your available balance is: <?php echo $balance . " BTN"; ?></div></h3>
    <br />
    <div class="container">
    <form id="form" action="transact.php" method="post">
      <!-- Payment options -->
      <input type="text" name="rec" class="form-control" placeholder="Receiver" size="55%" required><br>
      <input type="text" name="amount" class="form-control" placeholder="Amount" required><br>
      <input type="text" name="fee" class="form-control" placeholder="Fee (min 0.1)" required><br>
      ...
      if ($fcount > 1) {
       ...
       ?>
       <input class="btn btn-lg btn-primary btn-block" type="submit" name="Pay" value="Pay" />
    </form>
  </div>
    <?php
    if (isset($_GET["send"])) {
...
#      $_SESSION["thistory"][$transc] = $_GET["send"];
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if (strlen($extra) != 0 && strlen($pid) != 0) {
        ...
        echo "Are you sure you want to send " . $amount / 100 . " BTN to " . $rec . " with a fee of " . $fee / 100 . " BTN and an anonymity level of " . $anonymity . "<br><button onclick='" . $yeslink . "'>Yes</button><button onclick='" . $nolink . "'>No</button><button onclick='" . $later . "'>Confirm later</button>";
        }
